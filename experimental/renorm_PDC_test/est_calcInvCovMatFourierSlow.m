function V = est_calcInvCovMatFourierSlow(Rinv,E,foi,fs,M,p, verb)
%
...
%%
   ...
   Nfreq = length(foi);
   V = zeros(M,M,Nfreq,2,2);
   
   N = 2*Nfreq;
   H = Rinv;
   
   fvec = 2*pi*foi/fs; % 2*pi*((1:Nfreq)-1)/N
   
   interm= zeros(1, M, Nfreq, 2, 2);
   for k= 1:p
      ck= cos(fvec * k);
      sk= sin(fvec * k);
      for l= 1:p
         cl= cos(fvec * l);
         sl= sin(fvec * l);
         Hkl= H((k-1)*M+1:k*M, (l-1)*M+1:l*M);
         iii= zeros(1, M, Nfreq, 2, 2);
         COS= [ck.*cl; ck.*sl; sk.*cl; sk.*sl]';
         COS= reshape(COS,Nfreq,2,2);
         for d2= 1:M 
             % note that Vij depends on process i only via E(i,i)
             % therefore, interm need only be calculated for all j
            iii(1, d2, :, :, :)= Hkl(d2, d2)*COS;
         end
         interm= interm + iii;
      end
   end
