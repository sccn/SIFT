...
%%% state data recording to which you apply 20-s sliding-window         %%%
...
%%%          Makoto Miyakoshi 04/18/2021 SCCN, INC, UCSD                %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% EEG = pop_loadset;
EEG = pop_loadset('filename','continuous_icSelected.set','filepath','/var/local/makoto/Nick/p0100_import_upToDipfit/');
...
WindowLengthSec     = 20;                           % sliding window length in seconds.
WindowStepSizeSec   = 20;                           % sliding window step size in seconds
...
%% STEP 9: Export dDTF08 to an excel file.

% 04/18/2021 Makoto.
% Replace 'dDTF08' to 'RPDC' if you want to see RPDC.
% median value across sliding windows are used as a robust statistics.

% % Check from and to.
% testMatrix = [1 2 3; 4 5 6; 7 8 9];
% testMatrix(2,3) % 6, which is 'From IC3 To IC2'. Thus 'To-From' is the order.
% linearIdx = sub2ind([3 3],2,3);

%figure
graphEdgeCounter = 0;
outputTable = zeros(length(EEG.CAT.Conn.freqs), size(EEG.CAT.Conn.dDTF08,1)^2-size(EEG.CAT.Conn.dDTF08,1)+1);
edgeLabel = {};
for toIdx = 1:size(EEG.CAT.Conn.dDTF08,1)
    for fromIdx = 1:size(EEG.CAT.Conn.dDTF08,2)
        gridIdx = sub2ind([size(EEG.CAT.Conn.dDTF08,2) size(EEG.CAT.Conn.dDTF08,1)], toIdx, fromIdx);
        plotData = squeeze(median(EEG.CAT.Conn.dDTF08(toIdx, fromIdx,:,:),4));
        %subplot(size(EEG.CAT.Conn.dDTF08,2), size(EEG.CAT.Conn.dDTF08,1), gridIdx)
        %plot(EEG.CAT.Conn.freqs, plotData)
        
        graphEdgeCounter = graphEdgeCounter+1;
        
        if toIdx==1 & fromIdx==1
            edgeLabel{1,graphEdgeCounter} = 'Hz';
            outputTable(:,graphEdgeCounter) = EEG.CAT.Conn.freqs;
        end
        edgeLabel{1,graphEdgeCounter+1} = sprintf('To_IC%d_From_IC%d', toIdx, fromIdx);
        outputTable(:,graphEdgeCounter+1) = plotData;
    end
end

% Export the results into an Excel file.
exportingTable = array2table(outputTable, 'VariableNames', edgeLabel);
writetable(exportingTable, '/var/local/makoto/Nick/p0200_SIFT/dDTF08.xlsx', 'WriteVariableNames', true)
