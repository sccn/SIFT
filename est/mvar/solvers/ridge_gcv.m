...
arg_define([0 Inf],varargin, ...
    ...
    arg({'P','PriorCov'},[],[],'Prior covariance matrix for parameters. Can be an [nc x nc] matrix, where nc is the number of columns of A. Can also be a scalar, P, specifying the prior variance of each parameter (diagonal covariance matrix). If empty, identity covariance matrix assumed. A sparse matrix is advised if covariance matrix is not dense.'), ...
