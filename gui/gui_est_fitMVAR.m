function varargout = gui_est_fitMVAR(varargin)
%
% GUI_EST_FITMVAR M-file for gui_est_fitMVAR.fig
%      GUI_EST_FITMVAR, by itself, creates a new GUI_EST_FITMVAR or raises the existing
%      singleton*.
%
%      H = GUI_EST_FITMVAR returns the handle to a new GUI_EST_FITMVAR or the handle to
%      the existing singleton*.
%
%      GUI_EST_FITMVAR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_EST_FITMVAR.M with the given input arguments.
%
%      GUI_EST_FITMVAR('Property','Value',...) creates a new GUI_EST_FITMVAR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui_est_fitMVAR_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui_est_fitMVAR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gui_est_fitMVAR

% Last Modified by GUIDE v2.5 05-Jun-2012 19:34:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @gui_est_fitMVAR_OpeningFcn, ...
    'gui_OutputFcn',  @gui_est_fitMVAR_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before gui_est_fitMVAR is made visible.
function gui_est_fitMVAR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_est_fitMVAR (see VARARGIN)

set(hObject,'name','Autoregressive Model Fitting');
...
if isempty(varargin)
    error('You must pass ALLEEG to gui_est_fitMVAR');
end
...
uiwait(handles.gui_est_fitMVAR);


% --- Outputs from this function are returned to the command line.
function varargout = gui_est_fitMVAR_OutputFcn(hObject, eventdata, handles)
...
handles.ExitButtonClicked = 'Cancel';
guidata(hObject,handles);
uiresume(handles.gui_est_fitMVAR);
...
function gui_est_fitMVAR_ResizeFcn(hObject, eventdata, handles)
