...
function [m_norm surrogate_m] = est_pac(varargin)
...
[M N] = size(g.EEG.CAT.srcdata);

skip=ceil(N.*rand(g.numsurrogate*2,1)); 
skip(skip>maxskip)=[]; 
skip(skip<minskip)=[]; 
...
phase=angle(hilbert(eegfilt(g.EEG.icaact,g.EEG.srate,g.phaseband(1),g.phaseband(2))'));
