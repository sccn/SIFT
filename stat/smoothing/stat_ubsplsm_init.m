...
arg_define([0 Inf],varargin, ...
    arg_nogui({'T','TimeSeriesLength'},[],[],'Time series length. This is the length of each time-series (vector) to smooth'), ...
    arg_nogui({'R','NumTimeSeries'},[],[],'Number of observations. This is the total number of time-series to smooth. For instance, this might be the total number of connectivity edges across all subjects.'), ...
    arg({'K','Knots'},5,[],'Positions of spline knots. Two knots will be added to end-points. If K is a scalar, then we automatically assign (K+order-1) evenly spaced knots. A good heuristic is one knot every 5%','shape','row'), ...
...
if isempty(T) || isempty(R)
    error('sift:stat_ubsplsm_init','You must supply TimeSeriesLength and NumTimeSeries arguments');
end
...
if isscalar(K)
    ...
    knots = K(:)';
    K = length(knots)+2;  % we will add endpoints
end
   
if K-2 > T
    error('SIFT:stat_ubsplsm_init','The number of augmented knots (%d) cannot exceed the time-series length (%d).',K,T);
end

if verb
    fprintf(hlp_separator());
    fprintf('Generating %d splines for time-series of length %d\n',K,T);
    fprintf('Knot locations are %s\n',hlp_tostring(knots));
end

% Construct orthonormal basis functions
% -------------------------------------------------------------------------
phi_t = hlp_microcache('ubsplsm',@stat_ubsplsm_mkspl,knots,T,order,verb);
