...
SamplingRate = 100;      % Hz
Nl = 20*SamplingRate;                % length of each epoch (samples)
Nr = 1;                % number of trials (realisations)
ndisc = 1000;            % number of samples to discard from VAR model (startup transients)
TrueModelOrder = 2;      % VAR model order
ModelOrder =2;          % Model Order we will use
 
expr = {...
    sprintf('x1(t) = %s + e1(t)' ,sim_dampedOscillator(10,50,SamplingRate,1)) ... 
    sprintf('x2(t) = %s + e2(t)' ,sim_dampedOscillator(13,2,SamplingRate,2)) ... 
};
...
Nl = 5000;                % length of each epoch (samples)
Nr = 1;                % number of trials (realisations)
ndisc = 1000;            % number of samples to discard from VAR model (startup transients)
