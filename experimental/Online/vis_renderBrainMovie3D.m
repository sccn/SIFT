...
function [alltimepoints mov g] = vis_renderBrainMovie3D(ALLERSP,ALLITC,ALLCROSSF,ALLCROSSFANGLE,TIMES,FREQS,SELECTED,varargin)

if nargin < 6
    help vis_renderBrainMovie3D;
    return;
end;
...
%         set (gcf, 'visible', g.visible);
        ...
        if g.cortexTransparency < 1 && isempty(findobj(g.figurehandle,'tag','csfcortex'))
...
%             csfVertices = g.csf.vol.bnd(3).pnt;
            ...
            %         set(gcf,'visible','off')
            hold on;
            csfHandle = hlp_plotmesh(g.csf.faces, g.csf.vertices, [], false,hax);
