...
g = arg_define([0 1],varargin, ...
                ...
                arg_subtoggle({'usePriorState','UsePriorState'},[], ...
                {...
                    arg({'InitState','InitialState'},[],[],'Initial BSBL state object. If empty, defaults will be used.') ...
                },'Use previous state. The previously estimated state will be used as a starting estimate for BSBL. Alternately, you may provide an non-empty initial state structure via the ''InitialState'' argument.'), ...
...
% initialize state
if g.usePriorState.arg_selection && ~isempty(g.usePriorState.InitState)
    initAR = g.usePriorState.InitState;
elseif ~g.usePriorState.arg_selection
    % reset initAR
    initAR = struct([]);
end
