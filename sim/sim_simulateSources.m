...
g=arg_define([0 Inf],varargin,...
    ...
    arg({'channels','Channels'},[],[],'Channels to keep','type','cellstr','shape','row'), ...
    ...
    'gausspatch', ...
        ...
        arg({'sigma','Sigma'},10,[eps 100],'Patch standard deviation') ...
        }, ...
    'atlasroi', ...
        { ...
        arg({'roiAtlasLabels','RoiAtlasLabels'},{},ROINames,'ROI Labels','type','logical') ...
        }}, 'Source shape'), ...
    arg({'sourceAmps','SourceAmplitudes'},[],[],'Current density. This is a [num_sources x num_points] matrix of current density for each source','shape','matrix'), ...
    ...
    case 'gausspatch'
        if isempty(g.sourceShape.sourceCoords) && ~isempty(g.sourceShape.roiAtlasLabels)
            ...
            for k=1:numSources
                ...
                roiVertices = indices4Structure(hmObj,g.sourceShape.roiAtlasLabels{k})';
                ...
                elseif ~isfinite(posxyz)
                    fprintf('Could not compute the centroid of ROI ''%s''. Selecting first vertex instead.\n', ...
                            g.sourceShape.roiAtlasLabels{k});
                    posxyz = v(1,:);
...
scalpData = zeros(nchs,npnts);
srcData   = zeros(nvert,npnts);
for k=1:numSources
    ...
    J       = zeros(nvert,npnts);       % current density estimate
...
if g.addNoise.arg_selection
    scalpData = scalpData + (std(scalpData(:))./g.addNoise.snr).*randn(nchs,npnts);
end
